...
void MediaWidget::setupUI() {
    ...
    connect(waveformWidget, &WaveformWidget::doubleClicked, this, &MediaWidget::openSamplesFolderAndScan);
    ...
    connect(tableWidget, &QTableWidget::cellClicked, this, &MediaWidget::onRowClicked);
    connect(tableWidget, &QTableWidget::cellDoubleClicked, this, &MediaWidget::onRowDoubleClicked);
    connect(tableWidget, &CustomTableWidget::rowActivatedByKeyboard, this, &MediaWidget::onRowSelected);
    connect(tableWidget, &CustomTableWidget::enterPressedOnRow, this, &MediaWidget::onEnterPressed);
    connect(tableWidget, &CustomTableWidget::escapePressed, this, &QWidget::close);
    connect(tableWidget->horizontalHeader(), &QHeaderView::sectionClicked, this, &MediaWidget::handleHeaderClicked);

    f5Shortcut = new QShortcut(QKeySequence(Qt::Key_F5), this);
    connect(f5Shortcut, &QShortcut::activated, this, &MediaWidget::openSamplesFolderAndScan);
...
void MediaWidget::playSelectedSample() {
...
bool MediaWidget::parseWavHeader(const QString& filePath, qint64& fileSizeKB, quint32& sampleRate, quint16& channels, quint16& bitsPerSample, qint64& durationMs) {
    QFile file(filePath);
...
void MediaWidget::scanWavDirectory(const QString& folderPath) {
    QDir dir(folderPath);
...
void MediaWidget::handleHeaderClicked(int logicalIndex) {
...
void MediaWidget::onRowClicked(int row, int /*column*/) {
    onRowSelected(row);
}

void MediaWidget::onRowDoubleClicked(int row, int /*column*/) {
...
void MediaWidget::onRowSelected(int row) {
    if (mediaPlayer) {
        mediaPlayer->stop();
    }
...
void MediaWidget::onEnterPressed(int row) {
    QTableWidgetItem* item = tableWidget->item(row, 0);
    if (!item) return;
